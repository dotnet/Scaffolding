...
namespace Microsoft.Extensions.CodeGeneration.EntityFramework
{
    public class PropertyMetadata
    {
        ...
        public PropertyMetadata([NotNull]IProperty property, Type dbContextType)
        {
            ...
            var navigationProp = property.DeclaringEntityType
                .GetNavigations()
                .FirstOrDefault(nav => nav.Name == property.Name);

            if (navigationProp != null)
            {
                IsAssociation = true;
                var otherEntityType = navigationProp.ForeignKey.ResolveOtherEntityType(navigationProp.DeclaringEntityType);
                RelatedModelMetadata = new RelatedModelMetadata()
                {
                    EntitySetName = ModelMetadata.GetEntitySetName(dbContextType, otherEntityType.ClrType),
                    AssociationPropertyName = navigationProp.Name,
                    DisplayPropertyName = otherEntityType.DisplayName(), //This is probably not correct... what we want is display column not the display name of entity
                    TypeName = otherEntityType.ClrType.GetTypeInfo().FullName,
                    ShortTypeName = TypeName,
                    PrimaryKeyNames = otherEntityType.GetPrimaryKey().Properties.Select(pk => pk.Name).ToArray(),
                    FoeignKeyPropertyNames = navigationProp.PointsToPrincipal() ?
                        navigationProp.ForeignKey.Properties.Where(p => p.DeclaringEntityType == navigationProp.DeclaringEntityType).Select(p => p.Name).ToArray() :
                        new string[0]
                };
            }
