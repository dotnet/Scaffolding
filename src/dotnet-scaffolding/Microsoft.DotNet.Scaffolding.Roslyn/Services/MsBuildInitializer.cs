...
public class MsBuildInitializer
{
    ...
    private void RegisterMsbuild()
    {
        if (!MSBuildLocator.IsRegistered)
        {
            ...
            // None of the discovered SDK directories contained MSBuild.dll.
            // Fall back to the default MSBuild registration so workspace
            // creation does not fail with an unregistered MSBuild.
            _logger.LogInformation("No compatible SDK directory contained MSBuild.dll. Falling back to MSBuildLocator.RegisterDefaults().");
            MSBuildLocator.RegisterDefaults();
        }
    }
