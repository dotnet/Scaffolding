    ...
    private static IList<Type> BlazorEntraIdTemplateTypes
    {
        get
        {
            if (_blazorEntraIdTemplateTypes is null)
            {
                ...
                _blazorEntraIdTemplateTypes = allTypes.Where(t =>
                    !string.IsNullOrEmpty(t.FullName) &&
                    (t.FullName.Contains("AspNet.Templates.net10.BlazorEntraId") ||
                     t.FullName.Contains("AspNet.Templates.BlazorEntraId"))).ToList();
            }
