...
namespace Microsoft.DotNet.Scaffolding.Shared.Tests
{
    public class ProjectModifierHelperTests : DocumentBuilderTestBase
    {
        ...
        [Fact]
        public async Task GetStartupClassTests()
        {
            ...
            var programDocumentNoNamespace = new List<Document>() { CreateDocument(ProgramCsFileNoNamespace).WithName("Program.cs") };
            var programDocumentNoStartup = new List<Document>() { CreateDocument(ProgramCsFileNoStartup).WithName("Program.cs") };

            string startupName = await ProjectModifierHelper.GetStartupClass(programDocumentList);
            string startupName2 = await ProjectModifierHelper.GetStartupClass(programDocumentNoNamespace);
            string emptyStartupName = await ProjectModifierHelper.GetStartupClass(programDocumentNoStartup);
            string notStartupName = await ProjectModifierHelper.GetStartupClass(programDocumentDifferentStartupList);
            string nullStartup = await ProjectModifierHelper.GetStartupClass(null);

            Assert.Equal("Startup.cs", startupName);
            Assert.Equal("Startup.cs", startupName2);
            Assert.Equal("", emptyStartupName);
            Assert.Equal("", nullStartup);
            Assert.Equal("NotStartup.cs", notStartupName);
        }
        ...
        public async Task IsMinimalAppTests()
        {
            ...
            var programDocumentDifferentStartupList = new List<Document>() { CreateDocument(ProgramCsFileWithDifferentStartup).WithName("Program.cs") };
            var programDocumentNoNamespace = new List<Document>() { CreateDocument(ProgramCsFileNoNamespace).WithName("Program.cs") };
            Assert.True(await ProjectModifierHelper.IsMinimalApp(minimalProgramDocumentList));
            Assert.False(await ProjectModifierHelper.IsMinimalApp(programDocumentList));
            Assert.False(await ProjectModifierHelper.IsMinimalApp(programDocumentDifferentStartupList));
            Assert.False(await ProjectModifierHelper.IsMinimalApp(programDocumentNoNamespace));
        }
